...
WHERE revision_consumo.activo = 1 AND revision_consumo.id_telefono=$id_telefono";
...
<body>
    <div class="users-table">
        ...
        <table id="tablaRevO" class="display responsive nowrap" style="width:100%">    
                <?php
...
$(document).ready(function() {
   var table = $('#tablaRevO').DataTable({
    ...
    "oPaginate": {
        ...
        var filterValue = sessionStorage.getItem('filterValue_revO');
        if (filterValue) {
            table.search(filterValue).draw();
        }

        // Guardar el valor del filtro en localStorage cada vez que se busca
        $('#tablaRevO_filter input').on('input', function() {
            sessionStorage.setItem('filterValue_revO', $(this).val());
        });
         // Filtro por fecha
...
function eliminarFuncion(id) {
  var idTelefono = <?php echo $id_telefono; ?>;
  Swal.fire({
    title: "¿Estás seguro?",
    ...
    if (result.isConfirmed) {
      ...
      $.ajax({
        type: "GET",
        url: "/controlador/eliminarFuncion.php", // URL absoluta o relativa que se resuelva correctamente
        data: {
          tabla: "revision_consumo",
          id_columna: "id_revision_consumo",
          id: id,
          redirect: `/vista/index/idxRevTlfOne.php?id=${idTelefono}` // Utiliza plantillas de cadena para insertar la variable
        }
      }).done(function() {
        Swal.fire({
          title: "Eliminado!",
          ...
          if (result.isConfirmed) {
            window.location.href = `/vista/index/idxRevTlfOne.php?id=${idTelefono}`; // Utiliza plantillas de cadena para insertar la variable
          }
        });
      });
    }
