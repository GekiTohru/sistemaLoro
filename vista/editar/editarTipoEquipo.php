...
$conexion = $conexionObj->ConexionBD();
    ...
    $sql="SELECT tipo_equipo.*, tipo_equipo.id_tipo_equipo AS id
    FROM tipo_equipo
...
<body>
    <div class="users-table">
        <h2 style="text-align: center;">Editar tipo de equipo</h2>
        <div class="users-form">
        <form id="nuevo">
                <input type="hidden" name="id_tipo_equipo" value="<?= $row0['id']?>">
                ...
                <option value="USB" <?= $row0['teclado'] == 'USB' ? 'selected' : '' ?>>USB</option>
                <option value="Incorporado" <?= $row0['teclado'] == 'Incorporado' ? 'selected' : '' ?>>Incorporado</option>
                </select>
                </div>
</div>
    ...
    function editar() {
            $.ajax({
                type: 'POST',
                url: '../../controlador/editar/editarTipoEquipoFuncion.php',
                data: $('#nuevo').serialize(),
                success: function(data) {
            if (data === 'ok') {
                    Swal.fire({
                        icon: "success",
                        title: "Tipo de equipo editado correctamente",
                ...
                Swal.fire({
                    icon: "error",
                    title: "Error al editar el tipo de equipo",
