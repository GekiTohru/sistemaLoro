...
$conexion = $conexionObj->ConexionBD(); 
    ...
    $sql="SELECT tipo_almacenamiento.*, tipo_almacenamiento.id_almacentipo AS id
    FROM tipo_almacenamiento
    WHERE tipo_almacenamiento.id_almacentipo = $id_almacen";
...
<body>
    <div class="users-table">
        <h2 style="text-align: center;">Editar Tipo de almacenamiento</h2>
        <div class="users-form">
            <form id="nuevo">
            <input type="hidden" name="id" value="<?= $row0['id']?>">
                <div id="entradas" style="display: flex; flex-wrap: wrap;">
                <div class="inputs">
                <label for="nombre_almacentipo">Tipo de almacenamiento</label>
                <input type="text" name="nombre_almacentipo" id="nombre_almacentipo" placeholder="Ingrese el nombre" value="<?= $row0['nombre']?>" required>
                </div>
</div>
                <input type="submit" value="Editar Tipo de almacen.">
            </form>
        </div>
    ...
    function editar() {
            ...
            if (data === 'ok') {
                    Swal.fire({
                        icon: "success",
                        title: "Tipo de almacenamiento editado correctamente",
                ...
                Swal.fire({
                    icon: "error",
                    title: "Error al editar el tipo de almacenamiento",
