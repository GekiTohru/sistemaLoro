...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['data'])) {
    ...
    $pdf->Cell(70, 10, '', 0, 0);
    $pdf->Cell(10, 10, 'REGISTRO DE REVISIONES', 0, 1);
    $pdf->Ln();
    $pdf->setPrintHeader(false);
    $pdf->setPrintFooter(false);
    ...
    $pdf->Cell(30, 10, 'Área', 1, 0, 'C', 1);
    $pdf->Cell(30, 10, 'Consumo', 1, 0, 'C', 1);
    ...
    foreach ($data as $row) {
        checkPageBreak($pdf, 10);
        $fecha = DateTime::createFromFormat('Y-m-d', $row[6]);
        $fecha_formateada = $fecha->format('d-m-Y');
        $pdf->Cell(10, 10, $row[0], 1,0,'C');
        $pdf->Cell(30, 10, $row[1], 1,0,'C');
        $pdf->MultiCell(40, 10, $row[2], 1,'C',0,0);
        $pdf->MultiCell(30, 10, $row[3], 1,'C',0,0);
    ...
    $pdf->SetFont('helvetica', 'B', 10);
    $pdf->Cell(95, 10, 'TOTAL DE REVISIONES REALIZADAS: '.$contador, 1, 1,'C',1);
    $pdf->SetFont('helvetica', '', 7);
    $pdf->SetX(170);
    $pdf->MultiCell(25, 10, $filtro_fechas, 0, 'L');

    
    $pdf->Output('revisiones'.date('d_m_Y').'.pdf');
